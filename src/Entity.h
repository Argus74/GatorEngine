...
class Entity : public Serializable {
 ...
 public:
    ...
    std::shared_ptr<T> addComponent(
        TArgs&&... mArgs) {  // .. TArgs allows for any amount of components to be in the parameter
    ...
    template <typename T>
    void addComponent(
        const std::shared_ptr<T>& otherComponent) {  // if this errors out, try removing the const
        ...
        auto newComponent =
            std::make_shared<std::remove_reference_t<decltype(*otherComponent)>>(*otherComponent);
    ...
    template <std::size_t Index = 0, typename Func>
        ...
        Index<std::tuple_size<ComponentTuple>::value>::type forEachComponent(Func func) {
        func(std::get<Index>(components), Index);
        forEachComponent<Index + 1>(func);
    }
    template <std::size_t Index = 0, typename Func>
    typename std::enable_if<Index == std::tuple_size<ComponentTuple>::value>::type forEachComponent(
        Func) {
    ...
    std::shared_ptr<T> createComponentByName(const std::string& name) {
        if (name ==
            T::kComponentName) {  // Assuming each component class has a static `kComponentName` member
    ...
    void deserialize(const rapidjson::Value& value) override {
        for (auto it = value["components"].MemberBegin(); it != value["components"].MemberEnd();
             ++it) {
