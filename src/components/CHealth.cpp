...
CHealth::CHealth() {
    ...
    front_health_bar.setTexture(assetManager.GetTexturePrivate(
        "HealthBarFull"));  //For now hard code the assets used for health bar
...
void CHealth::Update() {
    ...
    front_health_bar.setTextureRect(sf::IntRect(original_texture_bounds.left,
                                                original_texture_bounds.top, visibleWidth,
                                                original_texture_bounds.height));

    sf::Color color = AssetManager::LerpColor(sf::Color::Red, sf::Color::Green, percentage);
    front_health_bar.setColor(color);
}

bool CHealth::UpdateHealth(
    float dmg) {  // Returns True if entity should be dead, returns false if not
    current_health += dmg;
