...
public:
    ...
    void deserialize(const rapidjson::Value& value) override {
		for (auto it = value["mouseMap"].MemberBegin(); it != value["mouseMap"].MemberEnd(); ++it) {
  	        if (it->value.IsInt()) {
  	            mouse_map[static_cast<sf::Mouse::Button>(std::stoi(it->name.GetString()))] = static_cast<Action>(it->value.GetInt());
            }
        }

		for (auto it = value["keyMap"].MemberBegin(); it != value["keyMap"].MemberEnd(); ++it) {
  	        if (it->value.IsInt()) {
  	            key_map[static_cast<sf::Keyboard::Key>(std::stoi(it->name.GetString()))] = static_cast<Action>(it->value.GetInt());
            }
        }
	}
};
