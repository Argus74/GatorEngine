    ...
    if (ImGui::BeginListBox(kExplorerLabel, ImVec2(ImGui::GetContentRegionAvail().x, -1))) {
        ...
        for (int i = 0; i < entityList.size(); i++) {
            ...
            std::string entityName =
                entity->hasComponent<CName>() ? entity->getComponent<CName>()->name : "NULL";
            ...
            const bool isSelected =
                Editor::active_entity_ == entity && Editor::state != Editor::State::Testing;
            std::string label =
                entityName + "##" + std::to_string(i);  // Prevent name conflcits bugs
...
void ExplorerWindow::DrawDropTarget(int targetIndex) {
    ...
    ImGui::InvisibleButton(kDropTargetLabel,
                           ImVec2(ImGui::GetContentRegionMax().x, kDropTargetHeight));
    if (ImGui::BeginDragDropTarget()) {
        // Draw line to indicate where entity will be inserted
        ImGui::SetCursorPosY(ImGui::GetCursorPosY() -
                             1.5 * kDropTargetHeight);  // IDK why this math works
        ImGui::Separator();
        ...
        if (payload) {
            ...
            if (sourceIndex < 0 || sourceIndex >= entityList.size() || sourceIndex == targetIndex)
                return;
